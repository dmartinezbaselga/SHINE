...
def read_scenario(batch_i, dir):
    path = dir + "/scenario_" + str(batch_i) + ".bin"
...
def preprocess_data(obstacles_previous_in, obstacles_vec_in, goals_in, human_truth_in, baseline_in, ours_in, 
                    human_previous_in):
    # Obstacles
    obstacles_previous = []
    # Previous positions
    for obs in obstacles_previous_in:
        obstacle_pos = []
        for i in range(len(obs.pos_x)):
            obstacle_pos.append(Position(obs.pos_x[i].cpu(), obs.pos_y[i].cpu()))
        obstacles_previous.append(obstacle_pos)
    # Future positions
    obstacles_vec = []
    for obs in obstacles_vec_in:
        obstacle_pos = []
        for i in range(len(obs.pos_x)):
            obstacle_pos.append(Position(obs.pos_x[i].cpu(), obs.pos_y[i].cpu()))
        obstacles_vec.append(obstacle_pos)
    # goals
    goals = []
    for i in range(len(goals_in.pos_x)):
        goals.append(Position(goals_in.pos_x[i].cpu(), goals_in.pos_y[i].cpu()))
    # human truth
    human_truth = []
    for i in range(len(human_truth_in.pos_x)):
        human_truth.append(Position(human_truth_in.pos_x[i].cpu(), human_truth_in.pos_y[i].cpu()))
    # Baseline traj
    baseline_traj = []
    for i in range(len(baseline_in.pos_x)):
        baseline_traj.append(Position(baseline_in.pos_x[i].cpu(), baseline_in.pos_y[i].cpu()))
    # Ours traj
    ours_traj = []
    for i in range(len(ours_in.pos_x)):
        ours_traj.append(Position(ours_in.pos_x[i].cpu(), ours_in.pos_y[i].cpu()))
    # human previous
    human_previous = []
    for i in range(len(human_previous_in.pos_x)):
        human_previous.append(Position(human_previous_in.pos_x[i].cpu(), human_previous_in.pos_y[i].cpu()))
    return obstacles_previous, obstacles_vec, goals, human_truth, human_previous, baseline_traj, ours_traj

def init_plot(ax, person_radius, obstacles_previous, goals, human_previous):
    ax.cla()
    global obstacle
    global goal_artist
    obstacles_artist = []
...
def plot_scenario_dynamic(obstacles_previous_in, obstacles_vec_in, goals_in, human_truth_in, human_previous_in, baseline_traj_in, ours_traj_in):
    person_radius = 0.25
    timestep = 0.4
    ...
    # ax = ax.reshape((-1))
    def update(i_video, obstacles_previous_in, obstacles_vec_in, goals_in, human_truth_in, human_previous_in, baseline_traj_in, ours_traj_in):
        global robot_arrow_artist, robot_ours, robot_baseline, obstacles_previous, obstacles_vec, goals, human_truth, human_previous, baseline_traj, ours_traj, obstacles_artist, obstacles_arrow_artist, robot_artist, robot_arrow_artist
        ...
        if i_video > 0 and i_video < len(human_previous):
            ...
            if i_video != 0:
                ...
                ax.add_artist(goal_artist)
                ax.legend([robot_artist, obstacle, goal_artist, robot_baseline, robot_ours], ['Robot', 'Obstacle', 'Goal', 'Baseline', 'Ours'], loc='lower right', markerscale=0.4)
        else:
            obstacles_previous, obstacles_vec, goals, human_truth, human_previous, baseline_traj, ours_traj = preprocess_data(obstacles_previous_in=obstacles_previous_in, obstacles_vec_in=obstacles_vec_in, goals_in=goals_in, human_previous_in=human_previous_in,
                                                                                                                              baseline_in=baseline_traj_in, ours_in=ours_traj_in, human_truth_in=human_truth_in)
            obstacles_artist, obstacles_arrow_artist, robot_artist, robot_arrow_artist, time = init_plot(ax, person_radius, obstacles_previous, goals, human_previous)


    anim = animation.FuncAnimation(fig, update, interval=0.8 * 1000, frames=len(human_previous_in.pos_x)+len(human_truth_in.pos_x), fargs=(obstacles_previous_in, obstacles_vec_in, goals_in, human_truth_in, 
                                                                                               human_previous_in, baseline_traj_in, ours_traj_in,))
    anim.running = True
