...
from trajectron.model.trajectron_guidance_Hcost import TrajectronGuidanceHCost

# torch.autograd.set_detect_anomaly(True)

def load_model(model_dir: str, device: str, epoch: int = 10, custom_hyperparams: Optional[Dict] = None,):
    while epoch > 0:
        load_path = pathlib.Path(model_dir) / f'model_registrar-{epoch}.pt'
...
def train(rank, args):
    ...
    if rank == 0 and not hyperparams["debug"]:
        ...
        elif hyperparams["train_data"] == "full_model":
            train_scene = "full_model"
        else:
    ...
    if hyperparams["train_data"] == "full_model":
        training_data = ["eupeds_eth-train", "eupeds_hotel-train", "eupeds_univ-train", "eupeds_zara1-train", "eupeds_zara2-train"]
        data_dirs = {"eupeds_eth": "~/datasets/eth_ucy_peds", 
        ...
        training_data = [hyperparams["train_data"]]
    
    if hyperparams["eval_data"] == "full_model":
        evaluating_data = ["eupeds_eth-val", "eupeds_hotel-val", "eupeds_univ-val", "eupeds_zara1-val", "eupeds_zara2-val"]
    else:
        evaluating_data = [hyperparams["eval_data"]]    

    train_dataset = UnifiedDataset(
        desired_data=training_data,
    ...
    if hyperparams["train_data"] == "full_model":
        base_model = glob.glob(f"kf_models/zara2_*_base_*")[0]
    else:
        base_model = glob.glob(f"kf_models/{train_scene}_*_base_*")[0]
    trajectron, _ = load_model(base_model, hyperparams["device"], epoch=5,
    ...
    trajectron = TrajectronGuidanceHCost(trajectron, model_registrar=model_registrar)
    # trajectron.set_environment()
    trajectron.set_annealing_params()
